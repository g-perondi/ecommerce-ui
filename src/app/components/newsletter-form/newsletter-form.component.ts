  ...
  template: `
    <div class="flex my-auto flex-col items-center justify-center bg-white/40 py-2 px-4 rounded-xl text-center backdrop-blur-md">
      ...
      <form class="mt-2 p-2 flex gap-2" (ngSubmit)="onSubmit()">
        ...
        <button
          type="submit"
          class="cursor-pointer rounded-sm w-25 bg-gradient-to-r from-sky-900 via-pink-700 to-amber-600 text-zinc-200 py-2 transition-colors hover:opacity-80 hover:shadow-lg hover:shadow-black/20">Subscribe</button>
      </form>
    </div>
  `,
  styles: []
